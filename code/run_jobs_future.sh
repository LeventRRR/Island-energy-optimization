...
MAIN_LOG="logs/main_future_log.log"
# --- 定义一个文件，专门记录未达到Gap目标的岛屿 ---
GAP_FAIL_LOG="logs/gap_failure_islands_future.csv"
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r ID Long Lat Country Island Pop Geometry Region; do
    ...
    # 1. 运行 disaster_future_2030 任务
    run_task_with_retry "disaster_future_2030" "disaster_future_2030.py" "${LOG_PREFIX}_disaster_future_2030.log"
    DISASTER_FUTURE_2030_EXIT=$?
    check_mip_gap "${LOG_PREFIX}_disaster_future_2030.log" "disaster_future_2030" "$ID" "$Lat" "$Long"
    
    # 2. 运行 disaster_future_2040 任务
    run_task_with_retry "disaster_future_2040" "disaster_future_2040.py" "${LOG_PREFIX}_disaster_future_2040.log"
    DISASTER_FUTURE_2040_EXIT=$?
    check_mip_gap "${LOG_PREFIX}_disaster_future_2040.log" "disaster_future_2040" "$ID" "$Lat" "$Long"

    # 3. 运行 disaster_future_2050 任务
    run_task_with_retry "disaster_future_2050" "disaster_future_2050.py" "${LOG_PREFIX}_disaster_future_2050.log"
    DISASTER_FUTURE_2050_EXIT=$?
    check_mip_gap "${LOG_PREFIX}_disaster_future_2050.log" "disaster_future_2050" "$ID" "$Lat" "$Long"
    ...
    # 检查所有任务的退出状态
    if [ $DISASTER_FUTURE_2030_EXIT -ne 0 ] || [ $DISASTER_FUTURE_2040_EXIT -ne 0 ] || [ $DISASTER_FUTURE_2050_EXIT -ne 0 ]; then
        TASK_SUCCESS=false
    fi
    ...
    if [ "$TASK_SUCCESS" = true ]; then
        ...
        echo "Island $ID: TASKS FAILED (2030:$DISASTER_FUTURE_2030_EXIT, 2040:$DISASTER_FUTURE_2040_EXIT, 2050:$DISASTER_FUTURE_2050_EXIT) at $ISLAND_END_TIME (Duration: $DURATION_FORMAT)" >> "$MAIN_LOG"
    fi
